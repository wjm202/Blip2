...
from paddlenlp.transformers.opt.modeling import OPTForCausalLM #debug
...
class Blip2OPTPaddle(Blip2Base):
    ...
    def __init__(
        ...
        self.opt_model = OPTForCausalLM.from_pretrained(opt_model,low_cpu_mem_usage=True,load_state_as_np=True)
    ...
    def generate(
        ...
        # import numpy as np
        # image = paddle.to_tensor(np.load("/paddle/workspace/wjm/LAVIS/image.npy"))[:1]
        ...
        attention_mask = paddle.concat([atts_opt, paddle.ones_like(opt_tokens.input_ids)], axis=1) # 16,32 16, 4->16,32
