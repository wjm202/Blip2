...
from paddlenlp.transformers.opt.modeling import TransformerDecoderLayer
...
class OPTEmbeddings(Layer):
    ...
    def forward_test(self, input_ids, input_embeddings):
    ...
    def forward(self, input_ids=None, position_ids=None, input_embeddings=None, query_embeds=None, past_key_values_length=0):
        #import pdb;pdb.set_trace()
        if input_ids is not None: # test
            input_shape = paddle.shape(input_ids)
            input_embeddings = self.word_embeddings(input_ids)
        else: # train
            input_shape = paddle.shape(input_embeddings)[:-1]
        ...
        if query_embeds is not None: 
            input_embeddings = paddle.concat([query_embeds, input_embeddings], axis=1)  # 80 36 2560 
            input_shape =  paddle.shape(input_embeddings)[:-1]
        # if position_ids is None:
        #     ones = paddle.ones(input_shape, dtype="int64")
        #     seq_length = paddle.cumsum(ones, axis=-1)
        #     position_ids = seq_length - ones
        if position_ids is None:  #train query_embeds is None
...
class OPTModel(OPTPretrainedModel):
    ...
    def forward(
        ...
        elif input_ids is not None: # train input_ids:None
            input_shape = paddle.shape(input_ids)
            input_ids = input_ids.reshape((-1, input_shape[-1])) # train: input_ids is None
        elif inputs_embeds is not None: # train input_ids:[2, 47, 2560]
        ...
        embedding_output, input_shape = self.embeddings(
            input_ids=input_ids, 
        ...
        #import pdb;pdb.set_trace() #input_shape 80,4 causal_mask [4，4] 对角线上都是-10000
        causal_mask = paddle.tensor.triu(paddle.ones((input_shape[-1], input_shape[-1])) * -1e4, diagonal=1)
        if past_key_values_length > 0: #past_key_values_length=0
        ...
        if attention_mask is not None: # 80*36   all0
            if len(attention_mask.shape) == 2:
                attention_mask = attention_mask[:, None, None, :] #train [2,44]
            attention_mask = attention_mask + causal_mask # 2,1,1,44 + 44,44
        else:
    ...
    def forward_bp(
        ...
        if query_embeds is not None: 
            embedding_output = paddle.cat([query_embeds, embedding_output], dim=1)  # 80 36 2560 
...
class OPTForCausalLM(OPTPretrainedModel):
    ...
    def forward(
        ...
        outputs = self.opt(
            ...
            query_embeds=query_embeds,
            position_ids=position_ids,
            attention_mask=attention_mask,# [80, 36] all 0
    ...
    def prepare_inputs_for_generation(self, input_ids, query_embeds=None, use_cache=False, cache=None, inputs_embeds=None, past=None, **kwargs):
    ...
    def prepare_inputs_for_generation_bp(self, input_ids, query_embeds=None, use_cache=False, cache=None, inputs_embeds=None, **kwargs):
        # only last token for inputs_ids if cache is defined in kwargs
        import pdb;pdb.set_trace()
